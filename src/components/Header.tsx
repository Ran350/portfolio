    ...
    <header>
      ...
      <div className="min-h-screen flex flex-col justify-between text-white">
        ...
        <div className="filter drop-shadow-lg">
          <h1 className="text-center text-6xl sm:text-7xl md:text-8xl font-telex">
            {title}
          </h1>
          ...
          <ul className="flex justify-center text-2xl">
            {subTitles.map((subtitle, index) => (
              <li
                key={subtitle}
                className={`p-2 border-current filter drop-shadow-lg ${
                  index !== 0 ? "border-l-2" : ""
                }`}
              >
                {subtitle}
          ...
          {[
            ...
            <a href={url} key={name} target="_blank" rel="noreferrer">
              <FontAwesomeIcon
                title={name}
                icon={icon}
                className="m-8 text-3xl filter drop-shadow-xl transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-110"
              />
            </a>
          ))}
        </div>
